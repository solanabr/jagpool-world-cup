  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-white/8">
      <div className="grid grid-cols-[2.5rem_1fr_3rem_5rem] gap-2 px-4 py-2.5 border-b border-white/8 bg-white/4">
        <span className="text-[10px] text-foreground/30 font-bold uppercase tracking-wider">Pos</span>
        <span className="text-[10px] text-foreground/30 font-bold uppercase tracking-wider">Validator</span>
        <span className="text-[10px] text-foreground/30 font-bold uppercase tracking-wider text-right">Plrs</span>
        <span className="text-[10px] text-foreground/30 font-bold uppercase tracking-wider text-right">Pts</span>
      </div>
      ...
      <div className="relative" style={{ height: SNAPSHOTS[0].length * ROW_H }}>
        {current.map((v, newIdx) => {
          ...
          const medal = newIdx === 0 ? "🥇" : newIdx === 1 ? "🥈" : newIdx === 2 ? "🥉" : null;
          return (
            <div
              key={v.name}
            ...
            >
              <span className="text-sm text-center">
                {medal ?? (
                  <span className="text-foreground/30 font-mono text-xs font-bold">{newIdx + 1}</span>
                )}
              </span>
              <span className="font-semibold text-sm truncate">{v.name}</span>
              <span className="text-right text-sm text-foreground/45 tabular-nums font-mono">{v.users}</span>
      ...
      <div className="border-t border-white/6 px-4 py-2.5 flex items-center justify-center gap-2">
        ...
        <p className="text-xs text-foreground/20">Preview · Live standings from Jun 11</p>
      </div>
    </div>
  );
}
